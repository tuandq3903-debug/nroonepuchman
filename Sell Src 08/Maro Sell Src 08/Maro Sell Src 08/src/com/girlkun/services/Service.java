...
//            e.printStackTrace();
    ...
    public void regisAccount(MySession session, Message _msg) {
        try {
            _msg.readUTF();
            _msg.readUTF();
            _msg.readUTF();
    ...
    public void Send_Info_NV(Player player) {
        if (player.zone == null) {
            return;
        }
        Message msg;
        try {
            if (!player.zone.getPlayers().isEmpty()) {
                for (Player plMap : player.zone.getPlayers()) {
                    ...
                    if (plMap.getSession() != null && plMap.getSession().version == 999) {
                        ...
                        msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.hp));
                        msg.writer().writeByte(0);//Hiệu ứng Ăn Đậu
                        msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.hpMax));
                    }
                ...
                msg = Service.getInstance().messageSubCommand((byte) 14);//Cập nhật máu
                msg.writer().writeInt((int) player.id);
                if (player.getSession() != null && player.getSession().version == 999) {
                    msg.writer().writeLong(player.nPoint.hp);
                    msg.writer().writeByte(0);//Hiệu ứng Ăn Đậu
                    msg.writer().writeLong(player.nPoint.hpMax);
                } else {
    ...
    public void Send_Info_NV_do_Injure(Player pl) {
        ...
        try {
            ...
            if (pl.getSession() != null && pl.getSession().version == 999) {
                ...
                msg.writer().writeInt(Util.fromLongtoInt(pl.nPoint.hp));
                msg.writer().writeByte(2);
                msg.writer().writeLong(Util.fromLongtoInt(pl.nPoint.hpMax));
            }
    ...
    public void sendInfoPlayerEatPea(Player player) {
        if (player.zone == null) {
            return;
        }
        Message msg;
        ...
        if (player.getSession() != null && player.isAdmin()) {
            ...
            if (text.equals("ad")) {
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1, "|7| ADMIN \n •⊹٭Ngọc Rồng One Puch Man٭⊹•\b|2| •⊹٭♥Bùi Kim Trường♥٭⊹•\b|4| Người Đang Chơi: " + Client.gI().getPlayers().size() + "\n" + "|8|Current thread: " + (Thread.activeCount())
                        ...
                        + "\n|7|CPU : " + cpuUsage + "/100%" + " ♥ " + "RAM : " + usedPhysicalMemoryStr + "/4GB"
                ...
                try {
                    ...
                    sendThongBao(player, "|7|" + player.name + " đã dịch chuyển tức thời đến: " + player.zone.map.mapName + " (" + player.zone.map.mapId + ")");
                    return;
                } catch (Exception e) {
        ...
        if (!player.isPet && !player.isBoss && !player.isNewPet) {
            ...
            try {
                ...
                if (player.getSession() != null && player.getSession().version == 999) {
                    ...
                    msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.hpg));
                    msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.mpg));
                    msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.dameg));
                    msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.hpMax));// hp full
                    msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.mpMax));// mp full
                    msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.hp));// hp
                    msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.mp));// mp
                    msg.writer().writeByte(player.nPoint.speed);// speed
                    msg.writer().writeByte(20);
                    msg.writer().writeByte(20);
                    msg.writer().writeByte(1);
                    msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.dame));// dam base
                    msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.def));// def full
                    msg.writer().writeByte(player.nPoint.crit);// crit full
                    msg.writer().writeLong(player.nPoint.tiemNang);
                    msg.writer().writeShort(100);
                    msg.writer().writeInt(Util.fromLongtoInt(player.nPoint.defg));
                    msg.writer().writeByte(player.nPoint.critg);
                }
                player.sendMessage(msg);
                msg.cleanup();
...
//    }
    ...
    public void hsChar(Player pl, long hp, long mp) {
        try {
            ...
            if (pl.zone != null) {
                for (int i = 0; i < pl.zone.getPlayers().size(); i++) {
                    Player plMap = pl.zone.getPlayers().get(i);
                    if (plMap != null) {
                        ...
                        if (plMap.getSession() != null && plMap.getSession().version == 999) {
                            ...
                            msg.writer().writeInt(Util.fromLongtoInt(hp));
                            msg.writer().writeInt(Util.fromLongtoInt(mp));
                        }
                        msg.writer().writeShort(pl.location.x);
                        msg.writer().writeShort(pl.location.y);
                        plMap.sendMessage(msg);
    ...
    public void sendThongBaoFromCc(Player player, String text) {
        sendBigMessage(player, 4335, text);
    }
