...
//            tlNeDon += RewardBlackBall.R6S_2;
    ...
    private void setHpMax() {
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUseBoHuyet) {
            this.hpMax += 500000;
            if (player.nPoint.power >= 100000000000L && player.nPoint.power <= 160000000000L) {
                this.hpMax += 100000;
            }
            if (player.nPoint.power >= 160000000000L && player.nPoint.power <= 180000000000L) {
                this.hpMax += 200000;
            }
            if (player.nPoint.power > 180000000000L) {
                this.hpMax += 400000;
            }
        }// item sieu cawsp
...
//        }
    ...
    private void setMpMax() {
        ...
        if (this.player.isPet && ((Pet) this.player).typePet == 2
                && ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA) {
            this.mpMax += ((long) this.mpMax * 8 / 100);//MP berus
        }
        if (this.player.isPet && ((Pet) this.player).typePet == 2
                && ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2) {
            this.mpMax += ((long) this.mpMax * 8 / 100);//MP berus
    ...
    public long getDameAttack(boolean isAttackMob) {
        ...
        dameAttack = (((long) Util.nextInt(95, 105) * dameAttack) / 100);
//        if (player.isPl()) {
