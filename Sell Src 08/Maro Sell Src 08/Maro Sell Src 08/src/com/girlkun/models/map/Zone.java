    ...
    public void pickItem(Player player, int itemMapId) {
        ...
        if (itemMap != null) {
            if (itemMap.playerId == player.id || itemMap.playerId == -1) {
                ...
                boolean picked = true;
                if (item.template.id == 934 && player.bongtai >= 4000) {
                    Service.getInstance().sendThongBao(player, "Mỗi ngày chỉ nhặt được 4000 mảnh hồn");
                    return;
                } else if (item.template.id == 934 && player.bongtai < 4000) {
                    player.bongtai++;
...
//                }
    ...
    private void infoPlayer(Player plReceive, Player plInfo) {
        ...
        try {
            ...
            if (plReceive.getSession() != null && plReceive.getSession().version == 999) {
                msg.writer().writeLong(plInfo.nPoint.hp);
                msg.writer().writeLong(plInfo.nPoint.hpMax);
            } else {
                msg.writer().writeInt(Util.fromLongtoInt(plInfo.nPoint.hp));
                msg.writer().writeInt(Util.fromLongtoInt(plInfo.nPoint.hpMax));
            }
...
//            Logger.logException(MapService.class, e);
        ...
        if (!plInfo.isBoss && !plInfo.isPet && !plInfo.isNewPet && !(plInfo instanceof BossDHVT) && !(plInfo instanceof Referee)) {
            ...
            if (plInfo.inventory.itemsBody.get(11).isNotNullItem()) {
                Service.getInstance().sendFoot(plInfo, (short) plInfo.inventory.itemsBody.get(11).template.id);
            }
        }
        ...
        try {
            ...
            for (Mob mob : mobs) {
                ...
                if (pl.getSession() != null && pl.getSession().version == 999) {
                    ...
                    msg.writer().writeInt(Util.fromLongtoInt(mob.point.gethp()));
                    msg.writer().writeByte(mob.level);
                    msg.writer().writeInt(Util.fromLongtoInt(mob.point.getHpFull()));
                }
