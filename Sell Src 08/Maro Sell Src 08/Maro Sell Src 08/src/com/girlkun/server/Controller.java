                ...
                case -7:
                    if (player != null) {
                        ...
                        try {
                            ...
                            int newX = toX;
                            int countValue = newX - oldX;
                            countValue = countValue > 0 ? countValue : countValue * -1;
                            if(toY < 200){
                                player.achievement.plusCount(5,countValue);
                            }
                        } catch (Exception e) {
                        }
...
//                    CombineService.gI().showInfoCombine(player, indexItem);
                ...
                case -45:
                    if (_session.player != null) {
                        SkillService.gI().useSkill(_session.player, null, null, _msg);
                    }
                    break;
                case -46:
                    if (player != null) {
...
//                                Service.gI().sendThongBao(player, "Đang bảo trì...");
                ...
                case 34:
                    if (_session.player != null) {
                        ...
                        SkillService.gI().selectSkill(_session.player, selectSkill);
                        if (selectSkill == Skill.SUPER_TRANFORMATION) {
                            SkillService.gI().useSkillTransformation(_session.player);
                        } else if (selectSkill == Skill.EVOLUTION) {
                            SkillService.gI().useSkillEvolution(_session.player);
                        }
                    }
                    break;
                case 54:
...
//        Service.gI().sendThongBaoOK(session, "Vui lòng đăng ký tài khoản tại trang chủ!");
    ...
    private void sendThongBaoServer(Player player) {
        Service.gI().sendThongBaoFromAdmin(player, "|7|"
                + "Chào Mừng " + player.name + " Đến Với Ngọc Rồng Kuroko\n" + "Ngọc Rồng Kuroko Không Chơi Phí Cả Thanh Xuân\n"
                + "Tại Map: " + "[" + player.zone.map.mapName + "]\n"
        );
    }
