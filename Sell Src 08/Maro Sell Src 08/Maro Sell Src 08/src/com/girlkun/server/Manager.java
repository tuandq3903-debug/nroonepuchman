...
//    public static byte RATE_EXP_SERVER = 1;// sau khi chinh
    ...
    public static final String queryTopNHS = "SELECT id, CAST( split_str(data_diem,',',1) AS UNSIGNED) AS data_nhs FROM player ORDER BY CAST( split_str(data_diem,',',1)  AS UNSIGNED) DESC LIMIT 20;";
    public static final String querytopGapThu = "SELECT id, CAST( split_str(data_diem,',',2) AS UNSIGNED) AS data_gt FROM player ORDER BY CAST( split_str(data_diem,',',2)  AS UNSIGNED) DESC LIMIT 20;";
    public static final String querytopTrungThu = "SELECT id, CAST( split_str(data_diem,',',3) AS UNSIGNED) AS data_thu FROM player ORDER BY CAST( split_str(data_diem,',',3)  AS UNSIGNED) DESC LIMIT 20;";
    public static final String querytopDoSat = "SELECT id, CAST( split_str(data_diem,',',4) AS UNSIGNED) AS data_ds FROM player ORDER BY CAST( split_str(data_diem,',',4)  AS UNSIGNED) DESC LIMIT 20;";
    ...
    public static List<TOP> topTrungThu;
    public static List<TOP> topDoSat;
    ...
    private void loadDatabase() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            while (rs.next()) {
                ...
                dataArray = (JSONArray) JSONValue.parse(rs.getString("createtimeLong"));
                ...
                dataArray.clear();
                dataArray = (JSONArray) JSONValue.parse(rs.getString("DataGas"));
...
//                    dataArray.clear();
            ...
            Logger.success("Finish load item Ký Gửi [" + ShopKyGuiManager.gI().listItem.size() + "]\n");
            ...
            topTrungThu = realTop(querytopTrungThu, con);
            Logger.success("Load Top Trung Thu Công [" + topTrungThu.size() + "]\n");
            topDoSat = realTop(querytopDoSat, con);
            Logger.success("Load Top Đồ Sát Công [" + topDoSat.size() + "]\n");
        ...
        Logger.log(Logger.RED_BOLD, "Tổng thời gian load database: " + Util.format(((double) (System.currentTimeMillis() - st) / 1000)) + "s\n");
    }   
    ...
    public static List<TOP> realTop(String query, Connection con) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                switch (query) {
                    case queryTopSM:
                        top.setInfo1("Tổng : " + Util.numberToMoney(rs.getLong("sm")));
                        top.setInfo2("Update Time : " + TimeUtil.getTimeNow("HH:mm:ss") + "\n" + "|7|[ Ngọc Rồng One Puch Man ]");
                        break;
                    case queryTopNV:
                        top.setInfo1(rs.getByte("nv") + " Nhiệm Vụ");
                        top.setInfo2("Update Time : " + TimeUtil.getTimeNow("HH:mm:ss") + "\n" + "|7|[ Ngọc Rồng One Puch Man ]");
                        break;
                    case queryTopSK:
                    ...
                    case queryTopPVP:
                        ...
                        break;
                    case queryTopNHS:
                        top.setInfo1(rs.getInt("data_nhs") + " điểm");
                        top.setInfo2("Update Time : " + TimeUtil.getTimeNow("HH:mm:ss") + "\n" + "|7|[ Ngọc Rồng One Puch Man ]");
                        break;
                    case querytopGapThu:
                        top.setInfo1(rs.getInt("data_gt") + " điểm");
                        top.setInfo2("Update Time : " + TimeUtil.getTimeNow("HH:mm:ss") + "\n" + "|7|[ Ngọc Rồng One Puch Man ]");
                        break;
                    case querytopTrungThu:
                        top.setInfo1(rs.getInt("data_thu") + " điểm");
                        top.setInfo2("Update Time : " + TimeUtil.getTimeNow("HH:mm:ss") + "\n" + "|7|[ Ngọc Rồng One Puch Man ]");
                        break;
                    case querytopDoSat:
                        top.setInfo1("Đã Đồ Sát Được: " + rs.getInt("data_ds") + " Người");
                        top.setInfo2("Update Time : " + TimeUtil.getTimeNow("HH:mm:ss") + "\n" + "|7|[ Ngọc Rồng One Puch Man ]");
                        break;
                }
                tops.add(top);
