...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            for (int i = 0; i < 100; i++) { // item tạo player 
                if (i == 0) { //thỏi vàng
            ...
            int[] skillsArr = gender == 0 ? new int[]{0, 1, 6, 9, 10, 20, 22, 19, 24, 27, 28}
                    : gender == 1 ? new int[]{2, 3, 7, 11, 12, 17, 18, 19, 25, 27, 28}
                    : new int[]{4, 5, 8, 13, 14, 21, 23, 19, 26, 27, 28};
            ...
            dataArray.add(0); //giáp xên
            dataArray.add(0); //giáp xên
            String trungThu = dataArray.toJSONString();
            dataArray.clear();

            dataArray.add(0); //bổ huyết
            dataArray.add(0); //bổ khí
            dataArray.add(0); //giáp xên
            String nuocMia = dataArray.toJSONString();
            dataArray.clear();
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, data_card, bill_data, data_item_time_sieu_cap,data_trung_thu,Nuoc_mia,info_achievement) "
                    ...
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, data_card, bill_data, itemTimeSC, trungThu, nuocMia, info_achive);
            ...
            try {
                ...
                if (player.isDie()) {
                    ...
                    if (MapService.gI().isMapDoanhTrai(mapId) || MapService.gI().isMapBlackBallWar(mapId)
                            ...
                            || MapService.gI().isMapKhiGas(mapId) || MapService.gI().isMapGiaiCuuMiNuong(mapId)
                            || MapService.gI().isMapConDuongRanDoc(mapId)
                            || MapService.gI().isMapVoDai(mapId)) {
                        mapId = player.gender + 21;
                        x = 300;
                        y = 336;
                    }
                ...
                dataArray.add(player.NguHanhSonPoint);
                dataArray.add(player.point_gapthu);
                dataArray.add(player.point_trungthu);
                dataArray.add(player.point_dosat);
                dataArray.add(player.chucNangNhanBua);
                dataArray.add(player.vip);
                dataArray.add(player.timevip);
                dataArray.add(player.point_kibi);
                ...
                dataArray.add(player.chuc_phuc_sd);
                dataArray.add(player.chuc_phuc_hp);
                dataArray.add(player.chuc_phuc_ki);
                String data_chucphuc = dataArray.toJSONString();
                dataArray.clear();

                dataArray.add(player.CheckDayOnl);
                dataArray.add(player.diemdanh);
                String diem_danh = dataArray.toJSONString();
                dataArray.clear();
                ...
                dataArray.add((player.itemTime.is1Trung ? (ItemTime.TIME_TRUNG_THU_10P - (System.currentTimeMillis() - player.itemTime.last1Trung)) : 0));
                dataArray.add((player.itemTime.is2Trung ? (ItemTime.TIME_TRUNG_THU_10P - (System.currentTimeMillis() - player.itemTime.last2Trung)) : 0));
                dataArray.add((player.itemTime.isgaQuay ? (ItemTime.TIME_TRUNG_THU_10P - (System.currentTimeMillis() - player.itemTime.lastgaQuay)) : 0));
                dataArray.add((player.itemTime.isthapCam ? (ItemTime.TIME_MAY_DO - (System.currentTimeMillis() - player.itemTime.lastthapCam)) : 0));
                String trungThu = dataArray.toJSONString();
                dataArray.clear();

                dataArray.add((player.itemTime.isnuocmiakhonglo ? (ItemTime.TIME_NUOC_MIA - (System.currentTimeMillis() - player.itemTime.lastnuocmiakhonglo)) : 0));
                dataArray.add((player.itemTime.isnuocmiathom ? (ItemTime.TIME_NUOC_MIA - (System.currentTimeMillis() - player.itemTime.lastnuocmiathom)) : 0));
                dataArray.add((player.itemTime.isnuocmiasaurieng ? (ItemTime.TIME_NUOC_MIA - (System.currentTimeMillis() - player.itemTime.lastnuocmiasaurieng)) : 0));
                String nuocMia = dataArray.toJSONString();
                dataArray.clear();
                ...
                String query = " update player set data_item_time_sieu_cap =?, head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, pointPvp=?, data_diem=?,data_card=?,bill_data =?,data_trung_thu=?,Nuoc_mia=?,diem_danh=?,isbienhinh=?,info_achievement=?,data_chucphuc=?,vodaisinhtu = ? where id = ?";
                GirlkunDB.executeUpdate(query,
                        ...
                        dataVoDaiSinhTu,
                        player.id);
                Logger.log(Logger.RED, player.name + " Lưu Xong! " + (System.currentTimeMillis() - st) + "\n");
            } catch (Exception e) {
