...
if ($validate !== false) {
    ...
    $stmt = $conn->prepare("SELECT giatri FROM trans_log WHERE status = 0 AND trans_id = ? AND pin = ? AND seri = ? AND type = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $giatri = $log['giatri']; // Lấy giá trị giatri từ kết quả truy vấn

        if ($status === 'thanhcong') {
            // Xử lý nạp thẻ thành công
            $price = $amount * $giatri; // Tính toán giá trị nạp thẻ bằng cách nhân amount với giatri
